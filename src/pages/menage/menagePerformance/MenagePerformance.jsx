...
export default function MenagePerformance() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("คุณต้องการที่จะลบข้อมูลใช่หรือไม่?")) {
      try {
        ...
        setPerformance(
          performance.filter((performance) => performance.id !== id)
        );
      } catch (error) {
        console.log(error);
      }
                ...
                {performance &&
                  performance.map((val, key) => (
                    <tr key={key}>
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                        <button
                          onClick={() => navigate(`/addperformance/${val.id}`)}
